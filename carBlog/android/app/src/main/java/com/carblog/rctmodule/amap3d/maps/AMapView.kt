...
import com.carblog.rctmodule.amap3d.toLatLng
import com.carblog.rctmodule.amap3d.toWritableMap
import com.carblog.rctmodule.amap3d.toLatLngBounds
...
class AMapView(context: Context) : TextureMapView(context) {
    ...
    private var hasPositionHere : Boolean = false
    var hasScrollviewParent : Boolean = false
    var onlyOneMarker : Boolean = false
    var addMarkerOnTap : Boolean = false
    ...
    init {
        ...
        map.setOnMapTouchListener{
            ...
            if(!this@AMapView.hasScrollviewParent){return@setOnMapTouchListener;}

            val p1 : ViewParent? = this@AMapView.parent
            if(p1 != null){
                if(p1 is ScrollView){
        ...
        map.setOnMapClickListener { latLng ->
            for (marker in markers.values) {
                marker.active = false
//                this@AMapView.remove(marker)
            }

            if(this@AMapView.onlyOneMarker){
                map.clear(true)
            }


            if(this@AMapView.addMarkerOnTap){
                val newMarker : Marker = this@AMapView.map.addMarker(MarkerOptions().position(latLng))
            }
...
//            if(this@AMapView.hasPositionHere==false){
//                this.hasPositionHere = true
//                this.map.moveCamera(CameraUpdateFactory.newCameraPosition(
//                        CameraPosition(
//                                LatLng(location.latitude,location.longitude), 15f, 30f, 0f
//                        )
//                ))
//            }
