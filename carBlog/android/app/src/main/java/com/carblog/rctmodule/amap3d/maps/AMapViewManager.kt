...
internal class AMapViewManager : ViewGroupManager<AMapView>() {
    ...
    fun setLanguage(view: AMapView, mapLanguage: Int) {
        view.map.setMapLanguage(if (mapLanguage == 1) {
            "en"
        } else {
            "zh_cn"
        })
    ...
    @ReactProp(name = "hasScrollviewParent")
    fun setHasScrollviewParent(view: AMapView, type: Boolean) {
        view.hasScrollviewParent = type
    }


    @ReactProp(name = "onlyOneMarker")
    fun setOnlyOneMarker(view: AMapView, type: Boolean) {
        view.onlyOneMarker = type
    }

    @ReactProp(name = "addMarkerOnTap")
    fun setAddMarkerOnTap(view: AMapView, type: Boolean) {
        view.addMarkerOnTap = type
    }
}
