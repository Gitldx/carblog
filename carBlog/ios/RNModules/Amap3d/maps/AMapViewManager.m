...
RCT_CUSTOM_VIEW_PROPERTY(onlyOneMarker, Bool, AMapView)
{
  BOOL b = [RCTConvert BOOL:json];
  view.onlyOneMarker = b;
}


RCT_CUSTOM_VIEW_PROPERTY(addMarkerOnTap, Bool, AMapView)
{
  BOOL b = [RCTConvert BOOL:json];
  view.addMarkerOnTap = b;
}
...
- (void)mapView:(AMapView *)mapView didSingleTappedAtCoordinate:(CLLocationCoordinate2D)coordinate {
  ...
  if(mapView.onlyOneMarker){
    [mapView removeAnnotations:mapView.annotations];
  }
  
  if(mapView.addMarkerOnTap){
    MAPointAnnotation *pointAnnotaiton = [[MAPointAnnotation alloc] init];
    [pointAnnotaiton setCoordinate:CLLocationCoordinate2DMake(coordinate.latitude,coordinate.longitude)];
