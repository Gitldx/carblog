...
RCT_EXPORT_VIEW_PROPERTY(onSingleTappedAtCoordinate, RCTBubblingEventBlock)
...
//    pointAnnotation.coordinate = CLLocationCoordinate2DMake(22.529321, 113.978006);
...
RCT_EXPORT_METHOD(setMarkers:(nonnull NSNumber *)reactTag :(nonnull NSDictionary *)params)
{
  dispatch_async(self.bridge.uiManager.methodQueue,^{
    [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
      id view = viewRegistry[reactTag];
...
- (void)mapView:(RCTAMap *)mapView didSingleTappedAtCoordinate:(CLLocationCoordinate2D)coordinate {
  if(mapView.onSingleTappedAtCoordinate) {
    mapView.onSingleTappedAtCoordinate(@{
                              @"data": @{
                                  @"Coordinate": @{
