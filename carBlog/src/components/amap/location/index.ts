...
import { Location, ReGeocode, AppKey, LocationMode, LocationPurpose, GeoLanguage, LocationPoint } from "./types";
 ...
 * @param listener
 */
export function addReGeoCodeListener(listener: (location: Location & ReGeocode) => void){
  return eventEmitter.addListener("onRegeocodeSearched",listener)
}
...
export function getAddress({latitude,longitude}){
  AMapGeolocation.getAddress({latitude,longitude});
}
...
export async function getDistance(location1:LocationPoint,location2:LocationPoint){
  const result = await AMapGeolocation.getDistance(location1,location2)
  return result;
}
